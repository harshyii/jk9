    ...
    grid.innerHTML = data.map(p => `
      <div class="col-6 col-md-4 col-lg-3">
        <div class="card h-100 border-0 shadow-sm rounded-0 product-card position-relative">
          <div class="position-absolute top-0 end-0 bg-warning text-dark font-monospace fw-bold px-2 py-0.5 small z-1">${p.brand || 'OEM'}</div>
          <img src="${p.img || 'https://images.unsplash.com/photo-1581092160607-ee22621dd758?w=400'}" class="card-img-top rounded-0 p-3 object-fit-contain" style="height: 180px; background: #fafafa;" alt="${p.name}">
          <div class="card-body d-flex flex-column p-3">
            <h6 class="card-title fw-bold text-dark text-truncate mb-1">${p.name}</h6>
            <p class="text-muted small mb-2 font-monospace">SKU: ${p.id}</p>
            <div class="mt-auto">
              <div class="d-flex align-items-baseline gap-2 mb-2">
                <span class="fs-5 fw-bold text-danger">${formatINR(p.price)}</span>
                ${p.mrp ? `<del class="text-muted small">${formatINR(p.mrp)}</del>` : ''}
              </div>
              <a href="#/product?id=${p.id}" class="btn btn-sm btn-outline-dark w-100 rounded-0 fw-bold">View Procurement Data</a>
            </div>
          </div>
        </div>
      </div>
